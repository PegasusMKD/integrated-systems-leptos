...
use crate::models::RegisterDetails;

use crate::constants::CONFIG;

async fn register(email: String, password: String) -> reqwest::Result<()> {
    let register_details = RegisterDetails::new(email, password);

    let client = reqwest::Client::new();
    let request = client.post(format!("{}/auth/register", CONFIG.api.path))
...
pub fn RegisterPage(cx: Scope) -> impl IntoView {
    ...
    let on_submit = move |ev: ev::SubmitEvent| {
        ...
        spawn_local(async move {
            let response = register(email.get_untracked(), password.get_untracked()).await;
            leptos::log!("{:?}", response);
            match response {
                Ok(_) => {
                    navigate("/login", NavigateOptions::default()).unwrap();
                },
                Err(err) => leptos::log!("Error during register: {:?}", err)
            };
        });
    };
                ...
                <div class="w-1/3">
                    ...
                    <form class="py-8 px-8" on:submit=on_submit>
                      ...
                      <div class="px-2 flex w-full justify-start items-center">
                        ...
                        <p class="px-6">Already have an account? <A href="/login">Login now!</A></p>
                      </div>
                    </form>
                </div>
            </div>

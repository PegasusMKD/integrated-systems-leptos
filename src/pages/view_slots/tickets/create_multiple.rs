...
#[component]
pub fn ViewSlotTicketsCreateMultiplePage(cx: Scope) -> impl IntoView {
    ...
    let count = create_rw_signal(cx, "0".to_string());

    let create_multiple = move |ev: ev::SubmitEvent| {
        ev.prevent_default();
        let navigate = use_navigate(cx);
        spawn_local(async move {
            let _ = create_ticket_for_view_slot(view_slot_id.get_untracked(), price.get_untracked(), count.get_untracked()).await;
            navigate(format!("/view-slots/tickets/{}", view_slot_id.get_untracked()).as_str(), NavigateOptions::default()).unwrap();
        });
    };

    view! {cx,
       <form class="py-4 px-8" on:submit=create_multiple>
          <div class="mb-6">
          ...
          <div class="mb-6">
            ...
            <input type="number" id="count" on:input=move |event| { count.set(event_target_value(&event)); } prop:value=move || { count.get() } class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block py-2.5 px-6 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="15" required/>
          </div>
